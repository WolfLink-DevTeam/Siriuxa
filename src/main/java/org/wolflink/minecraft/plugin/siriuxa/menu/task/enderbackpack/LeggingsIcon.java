...
@Singleton
public class LeggingsIcon extends ItemIcon {
    private static final ItemStack itemStack = EnderBackpack.getDefaultBackpack().getLeggings().clone();
    static {
        ItemMeta itemMeta = itemStack.getItemMeta();
        if(itemMeta != null) {
            itemMeta.setDisplayName("§8[ §a可放入护甲 §8]");
            itemMeta.setLore(Stream.of(" ","  §7可以放入 §f护腿 §7类型的护甲"," ").toList());
